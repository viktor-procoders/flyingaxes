...
const {BROWSERSYNC} = loadLocalConfig();
...
function loadLocalConfig() {
  log('Loading Local config file...');

  if (checkFileExists('config-local.yml')) {
    // config.yml exists, load it
    log(colors.bold(colors.cyan('config-local.yml')), 'exists, loading', colors.bold(colors.cyan('config-local.yml')));
    let ymlFile = fs.readFileSync('config-local.yml', 'utf8');
    return yaml.load(ymlFile);

  } else if (checkFileExists('config-local-default.yml')) {
    // config-local-default.yml exists, load it
    log(colors.bold(colors.cyan('config-local.yml')), 'does not exist, loading', colors.bold(colors.cyan('config-local-default.yml')));
    let ymlFile = fs.readFileSync('config-local-default.yml', 'utf8');
    return yaml.load(ymlFile);

  } else {
    // Exit if config.yml & config-default.yml do not exist
    log('Exiting process, no local config file exists.');
    log('Error Code:', err.code);
    process.exit(1);
  }
}
...
function styles() {
  return gulp.src(PATHS.entriestyles)
    ...
    .pipe($.if(REVISIONING && PRODUCTION, $.rev()))
    .pipe(gulp.dest(PATHS.dist + '/assets/css'))
    .pipe($.if(REVISIONING && PRODUCTION, $.rev.manifest()))
    .pipe(gulp.dest(PATHS.dist + '/assets/css'))
...
gulp.task('build',
  gulp.series(clean, gulp.parallel(styles, 'webpack:build', images, fonts, copy)));
